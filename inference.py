import os
import torch
import argparse
import sys
import json
import io
import re
import base64
from io import BytesIO
import pandas as pd
from PIL import Image
from datasets import load_dataset
from modelscope import AutoModel, AutoTokenizer, AutoModelForCausalLM
from swift.llm import (
    get_model_tokenizer, get_template, inference, ModelType,
    get_default_template_type, inference_stream
)
from swift.utils import seed_everything


parser = argparse.ArgumentParser(
    description="Script for self-correction learning (SCL) model inference.",
    formatter_class=argparse.ArgumentDefaultsHelpFormatter
)
parser.add_argument("--model", type=str, required=True, help="Name of the inference model.")
parser.add_argument("--prompt", type=int, default=2, help="ID of the self-correction prompt to use.")
parser.add_argument("--dataset", type=str, help="Inference dataset.")
parser.add_argument("--num_test", type=int, default=100, help="Number of test data points.")
args = parser.parse_args()


os.environ['CUDA_VISIBLE_DEVICES'] = '0'


# Load the specified model and tokenizer.
def load_model_and_tokenizer(model_name):
    if model_name == 'minicpm':
        model = AutoModel.from_pretrained('openbmb/MiniCPM-Llama3-V-2_5', trust_remote_code=True, torch_dtype=torch.float16).to('cuda')
        tokenizer = AutoTokenizer.from_pretrained('openbmb/MiniCPM-Llama3-V-2_5', trust_remote_code=True)
        model.eval()
        template = None
        
    elif args.model == 'internlm':
        model = AutoModelForCausalLM.from_pretrained( 'Shanghai_AI_Laboratory/internlm-xcomposer2-7b', trust_remote_code=True, torch_dtype=torch.float16).to('cuda')
        tokenizer = AutoTokenizer.from_pretrained('Shanghai_AI_Laboratory/internlm-xcomposer2-7b', trust_remote_code=True)
        model.eval()
        template = None
        
    elif args.model == 'llava7b':
        template_type = get_default_template_type('llava1_5-7b-instruct')
        model, tokenizer = get_model_tokenizer('llava1_5-7b-instruct', torch.float16, model_kwargs={'device_map': 'auto'})
        model = model.to('cuda')
        model.generation_config.max_new_tokens = 256
        template = get_template(template_type, tokenizer)

    else:
        raise ValueError(f"Unsupported model: {model_name}")
        
    return model, tokenizer, template

model, tokenizer, template = load_model_and_tokenizer(args.model)


# Load dataset based on the specified argument
def load_inference_dataset(dataset_name):
    dataset_map = {
        #'MMTBench': ('Kaining/MMT-Bench', 'train'),
        'MMStar': ('Lin-Chen/MMStar', 'val'),
        'MMBench': ('lmms-lab/MMBench_EN', 'dev'),
        'RealWorldQA': ('lmms-lab/RealWorldQA', 'test'),
        'SEEDBench': ('lmms-lab/SEED-Bench', 'test'),
        'ScienceQA': ('lmms-lab/ScienceQA-IMG', 'test')
    }
    
    if args.dataset == 'MMTBench':
      return pd.read_csv('./MMT-Bench/MMT-Bench_VAL.tsv', sep='\t').to_dict(orient='records')

    dataset_info = dataset_map.get(dataset_name)
    
    if dataset_info:
        return load_dataset(dataset_info[0])[dataset_info[1]]
    else:
        print('Invalid dataset.', file=sys.stderr)
        return None

ds = load_inference_dataset(args.dataset)


# Extracts the answer from the response string based on the dataset type.
def extract_answer(res):
    patterns = [
            r'##\s*([ABCD])\s*##',  
            r'##(.*?)##',           
            r'##\s*([0-3])\s*##'    
        ]
    
    for pattern in patterns:
        match = re.search(pattern, res)
        if match:
            return match.group(1).strip()
            
    match = re.search(r'([ABCD])\.', res)
    if match:
        return match.group(1).strip()
        
    return None


# Builds options string based on the dataset.
def build_options_str(dataset, example):
    if dataset == 'SEEDBench':
        return " A. {}. B. {}. C. {}. D. {}.".format(
            example['choice_a'], example['choice_b'], example['choice_c'], example['choice_d']
        )
    
    elif dataset == 'MMTBench':
        return " A. {}. B. {}. C. {}. D. {}.".format(
            example['A'], example['B'], example['C'], example['D']
        )
    
    elif dataset == 'MMBench':
        options_list = [f"{key}. {example.get(key)}" for key in ['A', 'B', 'C', 'D'] if example.get(key)]
        return " " + ' '.join(options_list)
    
    elif dataset == 'ScienceQA':
        choices = example['choices']
        return '\n' + '\n'.join([f"{index}. {choice}" for index, choice in enumerate(choices)])
    
    return ""


# Generates a review message based on the prompt type.
def get_review_msg(prompt):
    prompt_messages = {
        0: "Review your previous answer and find problems with your answer. Based on the problems you found, improve your answer.",
        1: "Review your previous answer and ensure that all relevant aspects of the image have been considered. Are there any elements or details that you missed? Based on your review, improve your answer.",
        2: "Review your contextual understanding of the image. Have you correctly interpreted the overall context and purpose of the scene? Based on your review, improve your answer.",
        3: "Review your answer and ensure that your understanding of the image is comprehensive and detailed. Are there any aspects of the scene that you have omitted or misinterpreted? Based on your review, improve your answer."
    }
    
    return prompt_messages.get(prompt, "") + " Explain your reasoning step-by-step."


QAs = dict()
correct_first = 0
correct_second = 0
total_questions = 0
formatted_QAs = []


for idx, example in enumerate(ds):
    correct_answer = str(example['answer'])
    if args.dataset == 'SEEDBench': 
      image = example['image'][0]
    elif args.dataset == 'MMTBench':
      image_base64 = example['image']
      image_decode = base64.b64decode(image_base64)
      image = Image.open(BytesIO(image_decode)).convert('RGB')
    else:
      image = example['image']

    question = example['question']
    options_str = build_options_str(args.dataset, example)
    question += options_str
    
    if args.dataset == 'ScienceQA':
        extractor = " Your final answer should be put between two ##, like ## 0 ## (if your final answer is 0), at the end of your response."
    else:
        extractor = " Your final answer should be put between two ##, like ## A ## (if your final answer is A), at the end of your response."
    
    question = question + "\nExplain your reasoning step-by-step." + extractor
    msgs = [{'role': 'user', 'content': question}]
    
    try:
        if args.model == 'minicpm':
            res_1 = model.chat(
                image=image, 
                msgs=msgs, 
                tokenizer=tokenizer, 
                sampling=True, 
                temperature=0.7
            )
            
        elif args.model == 'internlm':
            image = [model.vis_processor(image)]
            image = torch.stack(image)
            question = "<ImageHere> " + question
            with torch.cuda.amp.autocast():
                image = image.to(torch.float16)
                res_1, his = model.chat(
                    tokenizer, 
                    query=question, 
                    image=image, 
                    history=[], 
                    do_sample=True, 
                    temperature=0.7
                )
        
        elif args.model == 'llava7b':
            res_1, his = inference(model, template, question, images=[image])

        review_msg = get_review_msg(args.prompt) + extractor
        msgs.append({'role': 'assistant', 'content': res_1})
        msgs.append({'role': 'user', 'content': review_msg})
        
        QAs = {
            'index': idx,
            'Q1': {'role': 'user', 'content': question},
            'A1': {'role': 'assistant', 'content': res_1}
        }

        if args.model == 'minicpm':
            res_2 = model.chat(
                image=image, 
                msgs=msgs, 
                tokenizer=tokenizer, 
                sampling=True, 
                temperature=0.7
            )
            
        elif args.model == 'internlm':
            with torch.cuda.amp.autocast():
                res_2, history = model.chat(tokenizer, 
                                            query=review_msg, 
                                            image=image, 
                                            history=his, 
                                            do_sample=True, 
                                            temperature=0.7
                                           )
                
        elif args.model == 'llava7b':
            res_2, _ = inference(model, template, review_msg, history=his, images=[image, image])
            
        QAs['Q2'] = {'role': 'user', 'content': review_msg}
        QAs['A2'] = {'role': 'assistant', 'content': res_2}

        initial_answer = extract_answer(res_1) or res_1
        refined_answer = extract_answer(res_2) or res_2

        QAs['Initial answer'] = initial_answer
        QAs['Refined answer'] = refined_answer
        QAs['Correct answer'] = correct_answer
        
        if initial_answer == correct_answer:
            correct_first += 1
            if refined_answer == correct_answer:
                correct_second += 1
                QAs['Type'] = 1
            else:
                QAs['Type'] = 3
        else:
            if refined_answer == correct_answer:
                correct_second += 1
                QAs['Type'] = 2
            else:
                QAs['Type'] = 4
                
        total_questions += 1
        formatted_QAs.append(QAs)

        print("Question:", question)
        print("Res_1:", res_1)
        print("Res_2:", res_2)
        print("Initial answer:", initial_answer)
        print("Refined answer:", refined_answer)
        print("Correct answer:", correct_answer)
        print(f"Round {idx+1} - Accuracy - Initial: {correct_first / total_questions:.2%}")
        print(f"Round {idx+1} - Accuracy - Refined: {correct_second / total_questions:.2%}")

        if idx == args.num_test - 1:
            break
            
    except Exception as e:
        print(f"Error processing question {idx+1}: {e}")
        continue
    


        
if total_questions > 0:
    accuracy_first = correct_first / total_questions
    accuracy_second = correct_second / total_questions
    print(f"First answer accuracy: {accuracy_first:.2%}")
    print(f"Second answer accuracy: {accuracy_second:.2%}")

    jsonl_file = f'QAs_{args.model}_{args.dataset}_{args.prompt}_{args.num_test}.jsonl'
    with open(jsonl_file, 'w', encoding='utf-8') as f:
        for item in formatted_QAs:
            json.dump(item, f, ensure_ascii=False)
            f.write('\n')
    print(f"QAs successfully saved to {jsonl_file}.")
else:
    print("No valid questions processed.")
